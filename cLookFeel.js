...
// logic map in spreadsheet

function fGetemailForm(aEmail) {
  ...
  assert(
    vForms.length == 1,
    "fGetemailForm - only one special case known per type for " +
      aEmail +
      "/" +
      vForms.length
  );
  var aForm = vForms[0];
  assert("name" in aForm, "fGetemailForm the form does not have key name");
  assert("form" in aForm, "fGetemailForm the form does not have key form");
  delete avEmailForms[aEmail]; // and remove this special type

  return aForm;
}

/*
    Changed, 11/2 - For GBSC if delete
        Any Delete Permanently - route to EBB
        Any Delete normal - route to EBB, except GBSC goes to gbsc
    else add goes to gbsc
*/
//function fSpecialCaseAccounts(aForm, oTable)

/*
NOTE NOTE -- THIS NEEDS TO CHANGE AS IT DOESN'T SEEM RIGHT!
     CFS…
 1.       MT101 Send will need to go to CSFI:  csfi@acme.com  NOT NEEDED SINCE SPLIT
 2.       MT101 Receive will need to go to CSFI:  csfi@acme.com
 3.       Has BIC (whether it's MT101 or not) (aka,BESS Enrolment),will also need to go to DMC:  bessdata@acme.com (in addition to CSFI for MT101)
 4.       Does not have BIC (including SWIFT Payments & Statements) will need to also go to CMO:  ebbenrol@acme.com (in addition to CSFI for MT101)


Update 171221
Send MT101: Will go to CSFI and EBB
...
/*
    AEandIRX@acme.com; 
    then contents of drop-down for Symcor
    (rbcspinCalgary@symcor.com or rbcspinHalifax@symcor.com or rbcspinMontreal@symcor.com or rbcspinToronto@symcor.com or rbcspinVancouver@symcor.com or rbcspinWinnipeg@symcor.com)
*/
...
function fResetCommonArea() {
  // Common area - Show everything (that should be shown)
  fLog("fResetCommonArea");
  for (
    var iOrderInCommon = 0;
    iOrderInCommon < vaCommonSvcs.length;
    iOrderInCommon++
  ) {
    // services in area
    fQkDisplay(oCommonCB(kaCommonAddressForm, iOrderInCommon), false);
    fQkDisplay(oCommonCB(kaCommonContactForm, iOrderInCommon), false);
    fQkDisplay(oCommonCB(kaCommonCancelForm, iOrderInCommon), false);
  }
  fHide(oCommonEnrolmentText(kaCommonAddressForm));
  fHide(oCommonEnrolmentText(kaCommonContactForm));
  fHide(oCommonEnrolmentText(kaCommonCancelForm));

  var oCommCancelRBCx = oCommonCB(kaCommonCancelForm, 0);
  fDisplay(oCommCancelServicesRBCxSf, oCommCancelRBCx.rawValue == "1"); // Cancel RBCx - services section
}
...
function fTabSwitchSetInfo(iToTab) {
  ...
  if (bTab2 || bTab3) {
    ...
    // Can optimize - only do if showing...
    fCheckAccountTables(bTab2); // make sure they're okay based on tab one selection

    fFinallyShowInsideCommons(bTab2); // does the inside of the commons form

    //			Note show/hide Customized Template (Bulk eTransfer) for GBSC and for everyone else

    var iSegment = fGetClientType();
    fDisplay(
      oTab2Forms.Sect_BulkeTransfer.Content.sfCustomized.sfNotes.sfGBSCNote,
      iSegment == 1
    );
    fDisplay(
      oTab2Forms.Sect_BulkeTransfer.Content.sfCustomized.sfNotes.sfGTSNote,
      iSegment == 0
    );
  }
...
function fCopyGlobalsT1T3() {
  ...
  fReadGlobals(oTab2RBCx); // read tab two - RBCx only for now...
  fWriteGlobals(oTab2Forms); // write to tab 2/3
}

var oCCChkItems = [
  oCCCheckList.sfCopyOfInvoice,
  oCCCheckList.sfLetterOfDirection,
  oCCCheckList.sfVoidCheq,
  oCCCheckList.sfCheckDigitAlgorithm,
  oCCCheckList.sfGif,
  oCCCheckList.sfEDI,
  oCCCheckList.sfFlatFile,
  oCCCheckList.sf2944,
];
var vCCOn = [0, 0, 0, 0, 0, 0, 0, 0];

//rbcnet.fg.acme.com/assets/cb-ps-cash-mgmt/documents/formedi02944-doc.doc
http: function fCCCheckList() {
  // Triggers to turn on
  // a.     If Change in Corporate Creditor  name is selected then display “Copy of Invoice”
  var bChangeName = oCCFirst.sfNameChange.sfCB.cb.rawValue == 1;
  vCCOn[0] = bChangeName;

  // b.    If Change in Lead bank is selected and they select Yes for Concentrator client – then display  2 items, “Letter of Direction” and “RBC void cheque/ Deposit Slip/ Preauthorized debit form”
  // c.     If Change in Lead bank is selected and they select No for Concentrator client then display 1 item “RBC void cheque/ Deposit Slip/ Preauthorized debit form”
  var bChangeInLeadBank = oCCFirst.sfLeadBank.sfCB.cb.rawValue == 1;
  var bConcentrator =
    oCCFirst.sfLeadBank.sfWhenChecked.rbConcentrator.rawValue == 1;
  var bNonConcentrator = !bConcentrator;
  vCCOn[1] = bChangeInLeadBank && bConcentrator;
  vCCOn[2] = bChangeInLeadBank;

  // Some biz logic, forcing things later in Payment section
  if (bChangeInLeadBank && bConcentrator) {
    //  i.    Change in Payment Reporting/Delivery Method section to be mandatory.
    oCCFirst.sfPaymentReporting.sfCB.cb.rawValue = 1;
    fDisplay(oCCFirst.sfPaymentReporting.sfWhenChecked, true);
    // iii.    In the Change in Payment Reporting/Delivery Method section pre-select under choose on the selection of “Consolidated Reporting”
    oCCFirst.sfPaymentReporting.sfWhenChecked.sf.rbReporting = 2; // force to consolidated
  }

  // d.    If in the section Add/Remove Check Digit Routine , add is selected then display “ Check Digit Algorithm/Calculation”
  var bCheckDigit = oCCFirst.sfCheckDigit.sfCB.cb.rawValue == 1;
  var bCheckDigitAdd =
    oCCFirst.sfCheckDigit.sfWhenChecked.sfCDRQ.rb.rawValue == 1;
  vCCOn[3] = bCheckDigit && bCheckDigitAdd;

  // e.     If in section Add a Sample Image section, they selected Yes to the question then display “ GIF image of the Invoice”
  var bAddImage = oCCFirst.sfSampleImg.sfCB.cb.rawValue == 1;
  var bAddImageReally = oCCFirst.sfSampleImg.sfWhenChecked.sfQ.rb.rawValue == 1;
  vCCOn[4] = bAddImage && bAddImageReally;

  // f.     If in section Change in Payment reporting/Delivery Method , they select Consolidated reporting then display 2 items “Letter of Direction” and “EDI Enrolment form 2944”
  var bChangePayment = oCCFirst.sfPaymentReporting.sfCB.cb.rawValue == 1;
  var bConsolidated =
    oCCFirst.sfPaymentReporting.sfWhenChecked.sf.rbReporting.rawValue == 2;
  var bFlatFile =
    oCCFirst.sfPaymentReporting.sfWhenChecked.sfC.rbReportDelivery.rawValue ==
    1;
  var bEDI =
    oCCFirst.sfPaymentReporting.sfWhenChecked.sfC.rbReportDelivery.rawValue ==
    2;
  var bConcentratorReporting =
    oCCFirst.sfPaymentReporting.sfWhenChecked.sfC.rbConcentrator.rawValue ==
      1 ||
    oCCFirst.sfPaymentReporting.sfWhenChecked.sfC.rbConcentrator.rawValue == 2;
  vCCOn[1] |= bChangePayment && bConsolidated;
  vCCOn[5] = bChangePayment && bConsolidated && bEDI; // EDI
  vCCOn[6] = bChangePayment && bConsolidated && bFlatFile; // Flat File
  vCCOn[7] = bChangePayment && bConsolidated && bConcentratorReporting; // 2944 - any Concentrator

  //fLog("vCCON:"+vCCOn[0]+vCCOn[1]+vCCOn[2]+vCCOn[3]+vCCOn[4]+vCCOn[5]+vCCOn[6]+vCCOn[7]);
  var bAnyOn = false;
  for (i = 0; i < oCCChkItems.length; i++) {
    fDisplay(oCCChkItems[i], vCCOn[i]);
    bAnyOn |= vCCOn[i];
  }
  fDisplay(oCCCheckList, bAnyOn);

  // Oh, and also do some other random logic...
  /*
    Change in Lead Bank
    If Yes is selected for “is this concentrator (EDI) client” is selected, do the following
                                          i.    Change in Payment Reporting/Delivery Method section to be mandatory.
                                         ii.    Add a note that they must fill that section, something “ Please complete the Change In Payment reporting/Delivery Method section to select reporting option”
                                        iii.    In the Change in Payment Reporting/Delivery Method section pre-select under choose on the selection of “Consolidated Reporting”

    */

  // Primary contact - change required ...
  var bPRDCI = oCCFirst.sfContactInfo.sfWhenChecked.sf1.rbActions.rawValue == 1;
  if (bPRDCI) {
    //  i.    Change in Payment Reporting/Delivery Method section to be mandatory.
    oCCFirst.sfCPRDCI.sfCB.cb.rawValue = 1;
    fDisplay(oCCFirst.sfCPRDCI.sfWhenChecked, true);
  }
}
...
function fDisplay(o, bShow) {
  //  Call to show/hide an object
  if (bShow) fShow(o);
  else fHide(o);

  if (bShow)
    // recursively change validation here and below
    fEnableFieldsRecursively(o);
  else fDisableFieldsRecursively(o);
}
